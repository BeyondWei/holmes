 ...
 * under the License.
 */
package com.shuzheng.holmes.sdk.flume.log4jF;
 ...
 */
public class ShuzhengLog4jAppender extends AppenderSkeleton {
     ...
     */
    public ShuzhengLog4jAppender(String hostname, int port) {
    ...
    public synchronized void append(LoggingEvent event) throws FlumeException {
        ...
        if (!event.getLoggerName().contains("com.shuzheng.holmes.sdk.flume")) {
            try {
    ...
    private List<Event> parseEvents(LoggingEvent loggingEvent) {
        ...
        if (!StringUtils.isEmpty(logUuid)) {
            headers.put("holmes.logUuid", logUuid);
        }
        if (!StringUtils.isEmpty(projectUuid)) {
            headers.put("holmes.projectUuid", projectUuid);
        }
